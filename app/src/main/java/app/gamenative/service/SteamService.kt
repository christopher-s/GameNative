...
class SteamService : Service(), IChallengeUrlChanged {
    ...
    // PICS batches that exhaust retries are stashed here and re-queued on the next license
    // sync (which fires on every login), so a dropped batch is not lost permanently.
    private val picsRetryAppRequests = Collections.synchronizedList(mutableListOf<PICSRequest>())
    private val picsRetryPackageRequests = Collections.synchronizedList(mutableListOf<PICSRequest>())

    private fun drainPicsRetries(store: MutableList<PICSRequest>): List<PICSRequest> =
        synchronized(store) {
            if (store.isEmpty()) emptyList() else store.toList().also { store.clear() }
        }
...
//                        }
    ...
    private fun onLicenseList(callback: LicenseListCallback) {
        ...
        licenseListDebounceJob = scope.launch {
            ...
            // Re-queue PICS batches that exhausted retries since the last license sync.
            val retryPackages = drainPicsRetries(picsRetryPackageRequests)
            if (retryPackages.isNotEmpty()) {
                Timber.i("onLicenseList: re-queueing ${retryPackages.size} previously failed package(s) for PICS")
                retryPackages.chunked(MAX_PICS_BUFFER).forEach { chunk ->
                    packagePicsChannel.send(chunk)
                }
            }
            val retryApps = drainPicsRetries(picsRetryAppRequests)
            if (retryApps.isNotEmpty()) {
                Timber.i("onLicenseList: re-queueing ${retryApps.size} previously failed app(s) for PICS")
                retryApps.chunked(MAX_PICS_BUFFER).forEach { chunk ->
                    ...
                    while (picsAttempt <= MAX_PICS_BATCH_RETRIES) {
                        try {
                            ...
                            if (picsAttempt > MAX_PICS_BATCH_RETRIES) {
                                Timber.w("PICS product info request timed out for ${appRequests.size} app(s); max retries exceeded, will retry on next license sync")
                                picsRetryAppRequests.addAll(appRequests)
                            } else {
                ...
                .collect { packageRequests ->
                    ...
                    while (picsAttempt <= MAX_PICS_BATCH_RETRIES) {
                        ...
                        try {
                            ...
                            ensureActive()
                            Timber.w("PICS access token request timed out for ${queue.size} app(s); will retry package on next license sync")
                            picsRetryPackageRequests.addAll(packageRequests)
                        } catch (e: AsyncJobFailedException) {
                            Timber.w("Could not get PICS product info $e")
                        }
                    }
                            ...
                            picsAttempt++
                            if (picsAttempt > MAX_PICS_BATCH_RETRIES) {
                                Timber.w("PICS product info request timed out for ${packageRequests.size} package(s); max retries exceeded, will retry on next license sync")
                                picsRetryPackageRequests.addAll(packageRequests)
                            } else {
